...
int u_audio_set_capture_srate(struct g_audio *audio_dev, int srate)
{
  ...
  printk("Calling u_audio_set_capture_srate with srate %d\n", srate);
...
int u_audio_set_playback_srate(struct g_audio *audio_dev, int srate)
{
  ...
  printk("Calling u_audio_set_playback_srate with srate %d\n", srate);
	for (i = 0; i < UAC_MAX_RATES; i++) {
...
int u_audio_start_capture(struct g_audio *audio_dev)
{
	 ...
	 */
	prm->ffback = params->c_srate_active;
	u_audio_set_fback_frequency(audio_dev->gadget->speed,
				   ...
				   struct snd_ctl_elem_info *uinfo)
{
	...
	ffback_min = params->c_srate_active * FBACK_FREQ_MIN / 100;
	ffback_max = params->c_srate_active * FBACK_FREQ_MAX / 100;
